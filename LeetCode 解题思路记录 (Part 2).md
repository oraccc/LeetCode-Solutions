...
可以用一个栈，保存这些（进行乘除运算后的）整数的值。对于加减号后的数字，将其直接压入栈中；对于乘除号后的数字，可以直接与栈顶元素计算，并替换栈顶元素为计算后的结果。
...
生成了一个 1 到 49 的均匀随机数，因为：

- `(rand7() - 1)` 的结果是 0 到 6，乘以 7 后是 0、7、14、...、42
- 加上另一个 `rand7()` 的结果，最终 `ans` 的取值范围是 1 到 49，且每个数字出现的概率相同

接下来，程序只接受 `ans <= 40` 的情况，这样得到的 1 到 40 的随机数仍然是均匀分布的。最后，将这 40 个数字均匀地映射到 1 到 10 上，这样，每个结果（1 到 10）都有完全相同的概率。
